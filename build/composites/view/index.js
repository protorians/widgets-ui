import { Color, declarationExplodes, MainWidget, Section, Style } from "@protorians/widgets";
import { $ui } from "@protorians/core";
export function ThemeView(theme, declarations) {
    const { declaration, extended } = declarationExplodes(declarations, ['direction', 'helmet', 'navbar', 'bottomNavbar', 'footer', 'scrollable', 'title', 'styles']);
    ...
    return Section({
        ...
        .mount(({ widget }) => {
        $ui('body').forEach((e) => e.style.backgroundColor = `${widget.stylesheet.declarations.backgroundColor?.toString() || Color.tint}`);
    });
}
