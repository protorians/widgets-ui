...
export function ThemeAccordion(theme, { type, defaultIndex, children, direction, styles, outline }) {
    ...
    return theme.Layer({
        ...
        children: [
            children?.map(async (child) => {
                const { capability, current: accordion } = createCapability({
                ...
                capability.apply('open', (_index) => {
                    ...
                    if (!check) {
                        ...
                        if (isColumnDirection)
                            contentRef.current?.style({ height: `${size}px`, });
                        else
                            contentRef.current?.style({ width: `${size}px`, });
                ...
                capability.apply('toggle', (_index) => {
                    if (_index && typeof accordionEntries[_index] !== 'undefined') {
                        ...
                        if (accordion.status === AccordionStatus.Opened)
                            accordion.close();
                        else
                            accordion.open();
                    }
                ...
                const { content, index, trigger } = typeof child === 'function' ? child(accordion) : child;
